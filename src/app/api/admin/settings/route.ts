...
const schema = z.object({
  ...
  smtp: z.object({ host: text, port: text, username: text, password: text, fromName: text, noReplyEmail: z.union([z.literal(""), z.email()]), secure: z.boolean() }),
});
...
export async function PUT(request: Request) {
  try {
    ...
    await saveOperationalSettings({
      ...
      smtp: { ...input.smtp, password: input.smtp.password || current.smtp.password },
    });
